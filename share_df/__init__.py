...
def pandaBear(df: Union[pd.DataFrame, pl.DataFrame], use_iframe: bool = False, collaborative: bool = False, share_with: Union[str, list] = None) -> Union[pd.DataFrame, pl.DataFrame]:
    ...
    return start_editor(df, use_iframe=use_iframe, collaborative=collaborative, share_with=share_with)
...
class PandaBearAccessor:
    ...
    def __call__(self, use_iframe: bool = False, collaborative: bool = False, share_with: Union[str, list] = None):
        self._obj.update(pandaBear(self._obj, use_iframe=use_iframe, collaborative=collaborative, share_with=share_with))
        return None
...
def _register_polars_extension():
    if not hasattr(pl.DataFrame, "pandaBear"):
        class PolarsBearAccessor:
            ...
            def __call__(self, use_iframe: bool = False, collaborative: bool = False, share_with: Union[str, list] = None):
                modified_df = pandaBear(self._obj, use_iframe=use_iframe, collaborative=collaborative, share_with=share_with)
                self._obj.clear()
